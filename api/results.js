...
function reasonFor(shop, hearing) {
  ...
  const vibeMatch = !hearing.no_preference && arr(hearing.vibes).length > 0 && matchesAnyVibe(shop, hearing);
...
function select7Explorer(areaActive, hearing) {
  ...
  // S0: who + vibe
  uniquePush(picked, base.filter(s => matchesWho(s, hearing) && matchesAnyVibe(s, hearing)), 7);
  // S1: who only
  if (picked.length < 7) uniquePush(picked, base.filter(s => matchesWho(s, hearing)), 7);
  // S2: vibe only
  if (picked.length < 7) uniquePush(picked, base.filter(s => matchesAnyVibe(s, hearing)), 7);
  // S3: relaxed
...
export default async function handler(req, res) {
  try {
    ...
    // hearing を復元（あれば hearing を優先）
    let hearing = null;
    if (metadata.hearing) {
      try { hearing = JSON.parse(metadata.hearing); } catch(e) { hearing = null; }
    }

    // 互換：現行 metadata.area_groups でも動く
    if (!hearing) {
